...
def vs2_func_1(ns, traj, vs_params, bead_id):

    i, j, a = vs_params[:3]  # id, id, weight
    if a < 0 or a > 1:
...
def vs2_func_2(ns, traj, vs_params):

    i, j, a = vs_params[:3]  # id, id, nm

    for ts in ns.aa2cg_universe.trajectory:
        pos_i = ns.aa2cg_universe.atoms[i].position
        pos_j = ns.aa2cg_universe.atoms[j].position
        traj[ts.frame] = pos_i + a * (pos_j-pos_i) / np.abs(pos_j-pos_i)
...
def vs3_func_1(ns, traj, vs_params):

    i, j, k, a, b = vs_params[:5]  # id, id, id, nm, nm
...
def vs3_func_2(ns, traj, vs_params, bead_id):

    i, j, k, a, b = vs_params[:5]  # id, id, id, weight, nm
    if a < 0 or a > 1:
        msg = f"Virtual site ID {bead_id + 1} uses an incorrect weight. Expected weight in [0 , 1]."
        raise exceptions.MissformattedFile(msg)
...
def vs3_func_3(ns, traj, vs_params):

    i, j, k, ang_deg, d = vs_params[:5]  # id, id, id, degrees, nm
...
def vs3_func_4(ns, traj, vs_params):
...
# NOTE: tough to implement correctly, documentation seems incomplete
def vs4_func_2(ns, traj, vs_params):
...
def vsn_func_1(ns, traj, vs_params):

    for ts in ns.aa2cg_universe.trajectory:
        traj[ts.frame] = ns.aa2cg_universe.atoms[vs_params].center_of_geometry(pbc=None)


# vs_n func 2 -> Center of Mass
def vsn_func_2(ns, traj, vs_params, bead_id):
    ...
    for ts in ns.aa2cg_universe.trajectory:
        traj[ts.frame] = ns.aa2cg_universe.atoms[vs_params].center_of_mass(pbc=None)


# vs_n func 3 -> Center of Weights (each atom has a given weight, pairwise formatting: id1 w1 id2 w2 ..)
def vsn_func_3(ns, traj, vs_params):

    beads_ids = [bead_id for bead_id in range(0, len(vs_params), 2)]
    masses_and_weights = [ns.aa2cg_universe.atoms[i].mass * weight for weight in range(1, len(vs_params), 2)]

    for ts in ns.aa2cg_universe.trajectory:
        traj[ts.frame] = ns.aa2cg_universe.atoms[beads_ids].center(masses_and_weights)
