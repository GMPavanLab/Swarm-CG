...
def load_aa_data(ns):
    ...
    if ns.molname_in == None:
        ...
        molname_atom_group = ns.aa_universe.atoms[
        ...
        for i in range(len(molname_atom_group)):
            ...
            ns.all_atoms[atom_id] = {"conn": set(), "atom_type": atom_type, "atom_charge": atom_charge,
            ...
            if ndx_line != "":
                ...
                if bool(re.search("\[.*\]", ndx_line)):
                    ...
                    try:
                        ...
                        ns.all_beads[current_bead_id]["atoms_id"].extend(
...
def get_search_space_boundaries(ns):
    ...
    if ns.opti_cycle["nb_geoms"]["bond"] > 0:
        ...
        search_space_boundaries.extend(
            [[0, ns.default_max_fct_bonds_opti]] * ns.opti_cycle["nb_geoms"]["bond"])  # bonds force constants
    ...
    if ns.opti_cycle["nb_geoms"]["dihedral"] > 0:
        ...
        for grp_dihedral in range(ns.opti_cycle["nb_geoms"]["dihedral"]):  # dihedrals force constants
            if ns.cg_itp["dihedral"][grp_dihedral]["func"] == 2:
                search_space_boundaries.extend([[-ns.default_abs_range_fct_dihedrals_opti_func_without_mult,
                ...
                search_space_boundaries.extend([[-ns.default_abs_range_fct_dihedrals_opti_func_with_mult,
...
def get_initial_guess_list(ns, nb_particles):
    ...
    if ns.exec_mode == 1:
        for i in range(ns.opti_cycle["nb_geoms"]["constraint"]):
            input_guess.append(min(max(ns.out_itp["constraint"][i]["value"], ns.domains_val["constraint"][i][0]),
                                   ns.domains_val["constraint"][i][1]))  # constraints equilibrium values

        for i in range(ns.opti_cycle["nb_geoms"]["bond"]):
            input_guess.append(min(max(ns.out_itp["bond"][i]["value"], ns.domains_val["bond"][i][0]),
                                   ns.domains_val["bond"][i][1]))  # bonds equilibrium values

    for i in range(ns.opti_cycle["nb_geoms"]["bond"]):
        input_guess.append(
            min(max(ns.out_itp["bond"][i]["fct"], 0), ns.default_max_fct_bonds_opti))  # bonds force constants

    if ns.exec_mode == 1:
        for i in range(ns.opti_cycle["nb_geoms"]["angle"]):
            input_guess.append(min(max(ns.out_itp["angle"][i]["value"], ns.domains_val["angle"][i][0]),
                                   ns.domains_val["angle"][i][1]))  # angles equilibrium values

    for i in range(ns.opti_cycle["nb_geoms"]["angle"]):
        if ns.cg_itp["angle"][i]["func"] == 1:
            input_guess.append(
                min(max(ns.out_itp["angle"][i]["fct"], 0), ns.default_max_fct_angles_opti_f1))  # angles force constants
        elif ns.cg_itp["angle"][i]["func"] == 2:
            input_guess.append(
                min(max(ns.out_itp["angle"][i]["fct"], 0), ns.default_max_fct_angles_opti_f2))  # angles force constants

    if ns.exec_mode == 1:
        for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            input_guess.append(min(max(ns.out_itp["dihedral"][i]["value"], ns.domains_val["dihedral"][i][0]),
                                   ns.domains_val["dihedral"][i][1]))  # dihedrals equilibrium values

    for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
        if ns.cg_itp["dihedral"][i]["func"] == 2:
            input_guess.append(
                min(max(ns.out_itp["dihedral"][i]["fct"], -ns.default_abs_range_fct_dihedrals_opti_func_without_mult),
                    ns.default_abs_range_fct_dihedrals_opti_func_without_mult))  # dihedrals force constants
        else:
            input_guess.append(
                min(max(ns.out_itp["dihedral"][i]["fct"], -ns.default_abs_range_fct_dihedrals_opti_func_with_mult),
                    ns.default_abs_range_fct_dihedrals_opti_func_with_mult))  # dihedrals force constants
    ...
    if ns.opti_cycle["nb_cycle"] > 1:
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["constraint"]):
                if ns.all_best_emd_dist_geoms["constraints"][i] != config.sim_crash_EMD_indep_score:
                    ...
                    input_guess.append(
                        min(max(ns.out_itp["constraint"][i]["value"], ns.domains_val["constraint"][i][0]),
                            ns.domains_val["constraint"][i][1]))
        ...
        for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            if ns.all_best_emd_dist_geoms["dihedrals"][i] != config.sim_crash_EMD_indep_score:
                ...
                if ns.cg_itp["dihedral"][i]["func"] == 2:
                    input_guess.append(
                        min(max(ns.out_itp["dihedral"][i]["fct"],
                                -ns.default_abs_range_fct_dihedrals_opti_func_without_mult),
                            ns.default_abs_range_fct_dihedrals_opti_func_without_mult))
                else:
                    input_guess.append(min(
                        max(ns.out_itp["dihedral"][i]["fct"], -ns.default_abs_range_fct_dihedrals_opti_func_with_mult),
                        ns.default_abs_range_fct_dihedrals_opti_func_with_mult))
    ...
    # optionally second particle is initialized as input parameters ONLY at start of opti cycle 1
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["constraint"]):
                input_guess.append(
                    min(max(ns.out_itp["constraint"][i]["value_user"], ns.domains_val["constraint"][i][0]),
                        ns.domains_val["constraint"][i][1]))

        # bonds equilibrium values
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["bond"]):
                input_guess.append(min(max(ns.out_itp["bond"][i]["value_user"], ns.domains_val["bond"][i][0]),
                                       ns.domains_val["bond"][i][1]))
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["angle"]):
                input_guess.append(min(max(ns.out_itp["angle"][i]["value_user"], ns.domains_val["angle"][i][0]),
                                       ns.domains_val["angle"][i][1]))
        ...
        if ns.exec_mode == 1:
            for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
                input_guess.append(min(max(ns.out_itp["dihedral"][i]["value_user"], ns.domains_val["dihedral"][i][0]),
                                       ns.domains_val["dihedral"][i][1]))
        # dihedrals force constants
        for i in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            if ns.cg_itp["dihedral"][i]["func"] == 2:
                input_guess.append(
                    min(max(ns.out_itp["dihedral"][i]["fct_user"],
                            -ns.default_abs_range_fct_dihedrals_opti_func_without_mult),
                        ns.default_abs_range_fct_dihedrals_opti_func_without_mult))
            else:
                input_guess.append(min(
                    max(ns.out_itp["dihedral"][i]["fct_user"], -ns.default_abs_range_fct_dihedrals_opti_func_with_mult),
                    ns.default_abs_range_fct_dihedrals_opti_func_with_mult))
    ...
    for i in range(num_particle_random_start, nb_particles):
        ...
        if ns.exec_mode == 1:
            for j in range(ns.opti_cycle["nb_geoms"]["constraint"]):
                try:
                    ...
                    emd_err_fact = 1
                draw_low = max(ns.out_itp["constraint"][j][
                                   "value"] - config.bond_dist_guess_variation * ns.val_guess_fact * emd_err_fact,
                               ns.domains_val["constraint"][j][0])
                draw_high = min(ns.out_itp["constraint"][j][
                                    "value"] + config.bond_dist_guess_variation * ns.val_guess_fact * emd_err_fact,
                                ns.domains_val["constraint"][j][1])
        ...
        if ns.exec_mode == 1:
            for j in range(ns.opti_cycle["nb_geoms"]["bond"]):
                try:
                    ...
                    emd_err_fact = 1
                draw_low = max(ns.out_itp["bond"][j][
                                   "value"] - config.bond_dist_guess_variation * ns.val_guess_fact * emd_err_fact,
                               ns.domains_val["bond"][j][0])
                draw_high = min(ns.out_itp["bond"][j][
                                    "value"] + config.bond_dist_guess_variation * ns.val_guess_fact * emd_err_fact,
                                ns.domains_val["bond"][j][1])
                init_guess.append(draw_float(draw_low, draw_high, 3))
        ...
        for j in range(ns.opti_cycle["nb_geoms"]["bond"]):
            try:
                ...
                emd_err_fact = 1
            draw_low = max(min(ns.out_itp["bond"][j]["fct"] * (1 - ns.fct_guess_fact * emd_err_fact),
                               ns.out_itp["bond"][j]["fct"] - config.fct_guess_min_flat_diff_bonds), 0)
            draw_high = min(max(ns.out_itp["bond"][j]["fct"] * (1 + ns.fct_guess_fact * emd_err_fact),
                                ns.out_itp["bond"][j]["fct"] + config.fct_guess_min_flat_diff_bonds),
        ...
        if ns.exec_mode == 1:
            for j in range(ns.opti_cycle["nb_geoms"]["angle"]):
                try:
                    ...
                    emd_err_fact = 1
                draw_low = max(ns.out_itp["angle"][j][
                                   "value"] - config.angle_value_guess_variation * ns.val_guess_fact * emd_err_fact,
                               ns.domains_val["angle"][j][0])
                draw_high = min(ns.out_itp["angle"][j][
                                    "value"] + config.angle_value_guess_variation * ns.val_guess_fact * emd_err_fact,
                                ns.domains_val["angle"][j][1])
                init_guess.append(draw_float(draw_low, draw_high, 3))
        ...
        for j in range(ns.opti_cycle["nb_geoms"]["angle"]):
            try:
                ...
                emd_err_fact = 1
            draw_low = max(min(ns.out_itp["angle"][j]["fct"] * (1 - ns.fct_guess_fact * emd_err_fact),
                               ns.out_itp["angle"][j]["fct"] - config.fct_guess_min_flat_diff_angles), 0)
            if ns.cg_itp["angle"][j]["func"] == 1:
                draw_high = min(max(ns.out_itp["angle"][j]["fct"] * (1 + ns.fct_guess_fact * emd_err_fact),
                                    ns.out_itp["angle"][j]["fct"] + config.fct_guess_min_flat_diff_angles),
        ...
        if ns.exec_mode == 1:
            for j in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
                try:
                    ...
                    emd_err_fact = 1
                draw_low = max(ns.out_itp["dihedral"][j][
                                   "value"] - config.dihedral_value_guess_variation * ns.val_guess_fact * emd_err_fact,
                               ns.domains_val["dihedral"][j][0])
                draw_high = min(ns.out_itp["dihedral"][j][
                                    "value"] + config.dihedral_value_guess_variation * ns.val_guess_fact * emd_err_fact,
                                ns.domains_val["dihedral"][j][1])
                init_guess.append(draw_float(draw_low, draw_high, 3))
        ...
        for j in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            ...
            if ns.cg_itp["dihedral"][j]["func"] == 2:
                draw_low = max(min(draw_low, ns.out_itp["dihedral"][j][
                    "fct"] - config.fct_guess_min_flat_diff_dihedrals_without_mult),
                               ns.default_abs_range_fct_dihedrals_opti_func_without_mult)
                draw_high = min(max(draw_high, ns.out_itp["dihedral"][j][
                    "fct"] + config.fct_guess_min_flat_diff_dihedrals_without_mult),
                                ns.default_abs_range_fct_dihedrals_opti_func_without_mult)
            else:
                draw_low = max(min(draw_low, ns.out_itp["dihedral"][j][
...
def perform_BI(ns):
    ...
    if not ns.performed_init_BI["bond"] and ns.opti_cycle["nb_geoms"]["bond"] > 0:
        ...
        for grp_bond in range(ns.opti_cycle["nb_geoms"]["bond"]):
            ...
            if not 0 <= ns.out_itp["bond"][grp_bond]["fct"] <= min(config.default_max_fct_bonds_bi,
                                                                   ns.default_max_fct_bonds_opti):
                ns.out_itp["bond"][grp_bond]["fct"] = min(config.default_max_fct_bonds_bi,
                                                          ns.default_max_fct_bonds_opti) / 2

            if ns.verbose:
                print("  Bond group", grp_bond + 1, "estimated force constant:",
                      round(ns.out_itp["bond"][grp_bond]["fct"], 2))
    ...
    if not ns.performed_init_BI["angle"] and ns.opti_cycle["nb_geoms"]["angle"] > 0:
        ...
        for grp_angle in range(ns.opti_cycle["nb_geoms"]["angle"]):
            ...
            if func == 1:
                ...
                popt[0] = abs(popt[
                                  0])  # just to be safe, in case the fit yielded negative fct values but this is very unlikely since we provide good starting parameters for the fit
            ...
            # stay within limits in case user requires low force constants
            if func == 1:
                if not 0 <= ns.out_itp["angle"][grp_angle]["fct"] <= min(config.default_max_fct_angles_bi,
                                                                         ns.default_max_fct_angles_opti_f1):
                    ns.out_itp["angle"][grp_angle]["fct"] = min(config.default_max_fct_angles_bi,
                                                                ns.default_max_fct_angles_opti_f1) / 2
            elif func == 2:
                if not 0 <= ns.out_itp["angle"][grp_angle]["fct"] <= min(config.default_max_fct_angles_bi,
                                                                         ns.default_max_fct_angles_opti_f2):
                    ns.out_itp["angle"][grp_angle]["fct"] = min(config.default_max_fct_angles_bi,
                                                                ns.default_max_fct_angles_opti_f2) / 2

            if ns.verbose:
                print("  Angle group", grp_angle + 1, "estimated force constant:",
                      round(ns.out_itp["angle"][grp_angle]["fct"], 2))
    ...
    if not ns.performed_init_BI["dihedral"] and ns.opti_cycle["nb_geoms"]["dihedral"] > 0:
        ...
        for grp_dihedral in range(ns.opti_cycle["nb_geoms"]["dihedral"]):
            ...
            if func in config.dihedral_func_with_mult:
                multiplicity = ns.cg_itp["dihedral"][grp_dihedral][
            ...
            if ns.exec_mode == 1:  # in Mode 1, use the fitted value as equilibrium value (but stay within range)
                ...
                ns.out_itp["dihedral"][grp_dihedral]["value"] = np.rad2deg(
            ...
            if func in config.dihedral_func_with_mult:
                if not max(-config.default_abs_range_fct_dihedrals_bi_func_with_mult,
                           -ns.default_abs_range_fct_dihedrals_opti_func_with_mult) <= \
                       ns.out_itp["dihedral"][grp_dihedral]["fct"] <= min(
                        -config.default_abs_range_fct_dihedrals_bi_func_with_mult,
                        -ns.default_abs_range_fct_dihedrals_opti_func_with_mult):
                    ns.out_itp["dihedral"][grp_dihedral]["fct"] = np.sign(
                        ns.out_itp["dihedral"][grp_dihedral]["fct"]) * min(
                        config.default_abs_range_fct_dihedrals_bi_func_with_mult,
                        ns.default_abs_range_fct_dihedrals_opti_func_with_mult) / 2
            else:
                if not max(-config.default_abs_range_fct_dihedrals_bi_func_without_mult,
                    ...
                    ns.out_itp["dihedral"][grp_dihedral]["fct"] = np.sign(
                        ns.out_itp["dihedral"][grp_dihedral]["fct"]) * min(
                        config.default_abs_range_fct_dihedrals_bi_func_without_mult,
                        ns.default_abs_range_fct_dihedrals_opti_func_without_mult) / 2

            if ns.verbose:
                print("  Dihedral group", grp_dihedral + 1, "estimated force constant:",
                      round(ns.out_itp["dihedral"][grp_dihedral]["fct"], 2))
        ...
        for i in range(ncols):
            if i < ns.cg_itp["nb_constraints"]:
                ...
                if not ns.atom_only:
                    ...
                    print(
                        f"Constraint {grp_constraint + 1} -- AA Avg: {round(constraints[grp_constraint]['AA']['avg'], 3)} nm -- CG Avg: {round(constraints[grp_constraint]['G']['avg'], 3)}")
                else:
                    ax[nrow][i].set_title(
