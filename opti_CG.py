...
|                                                                                             |
|                 If you found Opti-CG useful in your research, please cite:                  |
|                Opti-CG: A general framework for automatic parametrization of                |
|                        bonded parameters in CG models via FST-PSO                           |
|        Empereur-mot C., Pesce L., Bochicchio D., Perego C., Pavan G. JCTC 2020 ASAP         |
|                                                                                             |
|                                Opti-CG relies on FST-PSO:                                   |
...
def compare_models(ns, manual_mode=True, ignore_dihedrals=False, calc_sasa=False, record_best_indep_params=False):
	...
	if manual_mode:
		...
		if ns.atom_only:
			...
			print()
			print('Radius of gyration (AA reference, NOT CG-mapped):', ns.gyr_aa)
	...
	if not ns.atom_only:
		...
		if ns.gyr_aa_mapped == None:
			...
			print()
			print('Radius of gyration (AA reference, no scaling, CG-mapped):', ns.gyr_aa_mapped, '+/-', ns.gyr_aa_mapped_std)

		compute_Rg(ns, traj_type='CG')
		print('Radius of gyration (CG model):', ns.gyr_cg, '+/-', ns.gyr_cg_std)
	...
	# record the max y for each geom type
		...
		for i in range(ncols):
			if i < ns.nb_constraints:
				...
				if not ns.atom_only:
					...
					if ns.verbose:
						print('Constraint '+str(grp_constraint+1)+' -- ATOM Avg: '+str(round(constraints[grp_constraint]['AA']['avg'], 3))+' nm -- CG Avg: '+str(round(constraints[grp_constraint]['CG']['avg'], 3))+' nm', flush=True)
				else:
					ax[nrow][i].set_title('Constraint grp '+str(grp_constraint+1)+' - Avg '+str(round(avg_diff_grp_constraints[grp_constraint], 3))+' nm')
					print('Constraint '+str(grp_constraint+1)+' -- ATOM Avg: '+str(round(constraints[grp_constraint]['AA']['avg'], 3)), flush=True)
		...
		for i in range(ncols):
			if i < ns.nb_bonds:
				...
				if not ns.atom_only:
					...
					if ns.verbose:
						print('Bond '+str(grp_bond+1)+' -- ATOM Avg: '+str(round(bonds[grp_bond]['AA']['avg'], 3))+' nm -- CG Avg: '+str(round(bonds[grp_bond]['CG']['avg'], 3))+' nm', flush=True)
				else:
					ax[nrow][i].set_title('Bond grp '+str(grp_bond+1)+' - Avg '+str(round(avg_diff_grp_bonds[grp_bond], 3))+' nm')
					print('Bond '+str(grp_bond+1)+' -- ATOM Avg: '+str(round(bonds[grp_bond]['AA']['avg'], 3)), flush=True)
				ax[nrow][i].grid(zorder=0.5)
				# ax[nrow][i].set_ylim(bottom=0)
				if ns.row_x_scaling:
		...
		for i in range(ncols):
			if i < ns.nb_angles:
				...
				if not ns.atom_only:
					...
					if ns.verbose:
						print('Angle '+str(grp_angle+1)+' -- ATOM Avg: '+str(round(angles[grp_angle]['AA']['avg'], 1))+'° -- CG Avg: '+str(round(angles[grp_angle]['CG']['avg'], 1))+'°', flush=True)
				else:
					ax[nrow][i].set_title('Angle grp '+str(grp_angle+1)+' - Avg '+str(round(avg_diff_grp_angles[grp_angle], 1))+'°')
					print('Angle '+str(grp_angle+1)+' -- ATOM Avg: '+str(round(angles[grp_angle]['AA']['avg'], 1)), flush=True)
				ax[nrow][i].grid(zorder=0.5)
				# ax[nrow][i].set_ylim(bottom=0)
				if ns.row_x_scaling:
		...
		for i in range(ncols):
			if i < ns.nb_dihedrals:
				...
				if not ns.atom_only:
					...
					if ns.verbose:
						print('Dihedral '+str(grp_dihedral+1)+' -- ATOM Avg: '+str(round(dihedrals[grp_dihedral]['AA']['avg'], 1))+'° -- CG Avg: '+str(round(dihedrals[grp_dihedral]['CG']['avg'], 1))+'°', flush=True)
				else:
					ax[nrow][i].set_title('Dihedral grp '+str(grp_dihedral+1)+' - Avg '+str(round(avg_diff_grp_dihedrals[grp_dihedral], 1))+'°')
					print('Dihedral '+str(grp_dihedral+1)+' -- ATOM Avg: '+str(round(dihedrals[grp_dihedral]['AA']['avg'], 1)), flush=True)
				ax[nrow][i].grid(zorder=0.5)
				# ax[nrow][i].set_ylim(bottom=0)
				if ns.row_x_scaling:
	...
	# now we have all the ylim max, so make them all consistent
	if ns.row_y_scaling:
		nrow = -1
		if ns.nb_constraints != 0:
			nrow += 1
			for i in range(ns.nb_constraints):
				ax[nrow][i].set_ylim(top=constraints_max_y)
		if ns.nb_bonds != 0:
			nrow += 1
			for i in range(ns.nb_bonds):
				ax[nrow][i].set_ylim(top=bonds_max_y)
		if ns.nb_angles != 0:
			nrow += 1
			for i in range(ns.nb_angles):
				ax[nrow][i].set_ylim(top=angles_max_y)
		if ns.nb_dihedrals != 0:
			nrow += 1
			for i in range(ns.nb_dihedrals):
				ax[nrow][i].set_ylim(top=dihedrals_max_y)
	...
	if not ns.atom_only:
		...
		all_dist_pairwise += '\n'
		print('Using bonds2angles scoring factor:', ns.bonds2angles_scoring_factor)
		print()
...
def eval_function(parameters_set, ns):
	...
	mini_killed, pre_md_killed, md_run_killed, new_best_fit = False, False, False, False
	...
	if os.path.isfile('mini.gro'):
		...
		# grompp -- pre-MD
		gmx_cmd = ns.gmx_path+' grompp -c mini.gro -p '+ns.top_input_basename+' -f '+ns.mdp_pre_md_basename+' -o pre-md'
		gmx_process = subprocess.Popen([gmx_cmd], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		gmx_out = gmx_process.communicate()[1].decode()
	...
	# TODO: remove
	if ns.nb_eval == 1:
		shutil.copytree(current_eval_dir, 'BI_initial_guess_eval_step_1')
	...
	if eval_score == ns.worst_fit_score:
		...
		print_stdout_forced('  Rg CG:  ', ' '+str(round(ns.gyr_cg, 2)), '   (Error abs.', str(round(abs(1-ns.gyr_cg/ns.gyr_aa_mapped)*100, 1))+'% -- Reference Rg AA-mapped:', str(ns.gyr_aa_mapped)+')')
		print_stdout_forced('  SASA CG:', ns.sasa_cg, '   (Error abs.', str(round(abs(1-ns.sasa_cg/ns.sasa_aa_mapped)*100, 1))+'% -- Reference SASA AA-mapped:', str(ns.sasa_aa_mapped)+')')
		# if ns.opti_cycle['nb_geoms']['dihedral'] == 0:
